...
def upload_image():
    try:
        ...
        for det in results.boxes:
            ...
            # ── Send Telegram alert if fire detected ──
            if cls_name == 'Fire' and conf > 0.75:
                fire_detected = True
        ...
        if fire_detected:
            send_telegram_alert("🔥 Fire Detected in uploaded IMAGE! Immediate action required.")
...
def gen_live_frames(video_path):
    ...
    last_alert_time = 0  # ── Tracks last alert timestamp
    ...
    while cap.isOpened():
        ...
        for det in results.boxes:
            ...
            if cls_name == 'Fire' and conf > 0.75 and time.time() - last_alert_time > 5:
                send_telegram_alert(f"🔥 Fire Detected in VIDEO stream! Confidence: {conf:.2f}")
...
def video_feed():
    ...
    if not video_path or not os.path.exists(video_path):
        return "No uploaded video available for streaming", 400
